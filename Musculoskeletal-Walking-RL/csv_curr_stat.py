...
NAME = "history" + CONFIGURATION.get('NAME','default') + ".csv"
...
@nrp.Robot2Neuron()
def csv_curr_stat(t, recorder, Height, reward, nb_ep, pos_x, pos_z, vel_x, vel_z):
	if pos_z.value is not None:
			recorder.record_entry(nb_ep.value,Height.value,reward.value, pos_x.value, pos_z.value, vel_x.value, vel_z.value)
