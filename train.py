...
def sim_start(experiment,server,config_file_path,results_save_path):
	...
	#iteration over episodes
		...
		height = 1
		total_reward = 0
		while(itr_idx<MAX_ITER and height>0.5 and height<1.5):
	...
	history.to_csv(results_save_path+'/'+conf.split('/')[-1][:-5]+'.csv')
	import seaborn as sns
	sns.kdeplot(history.reward)
...
parser.add_argument('--results_path','-r',help='path to simulatuion results. frfault: ./results', default='./results')
